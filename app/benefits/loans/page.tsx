...
interface EmployeeLoan {
  ...
  payment_term?: "monthly" | "semi-monthly" | null;
  ...
  async function handleSubmit(e?: React.MouseEvent) {
    ...
    try {
      ...
      const loanData = {
        ...
        payment_term: formData.deduct_bi_monthly ? "semi-monthly" : "monthly",
      ...
      if (editingLoan) {
        ...
        if (created?.id) {
          const paymentTerm: LoanPaymentTerm = formData.deduct_bi_monthly
            ? "semi-monthly"
            : "monthly";
          const perInstallment = perInstallmentFromHeader({
            monthlyPayment: parseFloat(formData.monthly_payment),
            paymentTerm,
            deductBiMonthly: formData.deduct_bi_monthly,
          });
          const installmentCount =
            paymentTerm === "semi-monthly"
              ? parseInt(formData.total_terms, 10) * 2
              : parseInt(formData.total_terms, 10);
          const installments = generateLoanInstallments({
            effectivityDate: formData.effectivity_date,
            installmentCount,
            perInstallment,
            originalBalance: parseFloat(formData.original_balance),
            paymentTerm,
            cutoffAssignment: formData.cutoff_assignment,
          });
          if (installments.length) {
            ...
            if (schedError) {
              console.error("Schedule insert error:", schedError);
              toast.error(
                "Loan saved, but installment schedule failed. Edit the loan or re-import."
              );
            }
          }
        }
  ...
  async function loadPaymentHistory(loan: EmployeeLoan) {
    ...
    try {
      const { data: posts, error: postsError } = await supabase
        ...
        .select(
          "amount, balance_before, balance_after, created_at, run_id"
        )
        .eq("loan_id", loan.id)
        .order("created_at", { ascending: true });
      if (postsError) throw postsError;

      const runIds = [...new Set((posts ?? []).map((row) => row.run_id as string).filter(Boolean))];
      const runById = new Map<string, { period_start: string; period_end: string }>();
      if (runIds.length) {
      ...
      if (posts?.length) {
        setPaymentHistory(
          posts.map((row) => {
            ...
            return {
              payslip_id: row.run_id,
              payslip_number: "Register",
              period_start: run?.period_start ?? row.created_at,
              period_end: run?.period_end ?? row.created_at,
              payment_amount: Number(row.amount) || 0,
              running_balance: Number(row.balance_after) || 0,
                    ...
                    {pagedLoans.map((loan) => (
                      <div key={loan.id} className={dbMobileListCard}>
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <DashboardMobileField
                            label="Terms left"
                            value={`${loan.remaining_terms} / ${loan.total_terms}`}
                          />
                          <DashboardMobileField
                            label="Next due"
                            value={
                              loan.next_due
                                ? `${format(new Date(`${loan.next_due}T00:00:00`), "MMM dd, yyyy")}${
