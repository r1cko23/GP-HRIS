...
export default function PayslipsPage() {
  ...
  // Deep-link from Payroll Run: /payroll/payslips?employee=<uuid>&period=YYYY-MM-DD
  useEffect(() => {
  ...
  async function loadAttendanceAndDeductions() {
    ...
    try {
      ...
      const contextRes = await fetch(`/api/payroll/payslips/context?${contextQs}`);
      if (!contextRes.ok) {
      ...
      toast.error("Failed to load attendance/benefits/deductions");
    }
  }
    ...
    try {
      ...
      for (const loanRecord of loansData) {
        const { count: scheduled } = await supabase
          ...
          .eq("loan_id", (loanRecord as any).id)
          .eq("status", "pending");
        if ((scheduled ?? 0) > 0) {
          console.log(
            `Loan ${(loanRecord as any).id} is tracked by Organic payroll schedules; skipping weekly balance post`
          );
          continue;
        }
  ...
  const cutoffStatutory = useMemo(() => {
    if (monthlySalary <= 0) return emptyCutoffStatutory();
    return getCutoffStatutoryDeductions(monthlySalary);
  }, [monthlySalary]);
  const govDed = cutoffStatutory.total;
                ...
                <VStack gap="3">
                  ...
                  <VStack gap="2" align="start">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
                      {(() => {
                        const sssRegularShown = cutoffStatutory.sss_regular;
                        const sssWispShown = cutoffStatutory.sss_wisp;
                        const philhealthShown = cutoffStatutory.philhealth;
                        ...
                        return (
                          <>
                            ...
                            >
                              ...
                              <span className="font-semibold text-sm ml-2 flex-shrink-0">
                                {formatCurrency(cutoffStatutory.pagibig)}
                              </span>
                            </HStack>
                          </>
                        );
                    ...
                    <PayslipPrint
                      ...
                      deductions={{
                        ...
                        monthlyLoans: isFirstCutoff()
                          ...
                          : undefined,
                        sssContribution: cutoffStatutory.sss_regular,
                        sssWisp: cutoffStatutory.sss_wisp,
                        philhealthContribution: cutoffStatutory.philhealth,
                        pagibigContribution: cutoffStatutory.pagibig,
                        withholdingTax: (() => {
