...
export default function IncentiveAuditPage() {
  ...
  const loadUploads = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/reports/incentive-audit/upload");
      if (!res.ok) {
  ...
  const loadUploadDetail = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/reports/incentive-audit/upload?id=${id}`);
      if (!res.ok) {
        const err = await res.json();
  ...
  async function handleClearAll() {
    setClearingAll(true);
    try {
      const res = await fetch("/api/reports/incentive-audit/upload?clear_all=true", {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json();
