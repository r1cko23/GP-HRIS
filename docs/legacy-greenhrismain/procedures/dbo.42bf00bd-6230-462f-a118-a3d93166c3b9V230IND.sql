...
CREATE PROCEDURE [dbo].[42bf00bd-6230-462f-a118-a3d93166c3b9V230IND]
...
SELECT employee_id,sssbasis FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment AND employee_id = @idemployee  order by idpayrollsum desc
...
--SET basic prOCESS---------------------------------------------------------
			...
			End
			else if @sssbasis = 'basic' and @countempidsss >1 and @withsss=1 
		  	BEgin
...
SELECT employee_id FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment AND employee_id = @idemployee  order by idpayrollsum
   ...
   FETCH NEXT FROM myCursor7 INTO @idemployeewtax
	...
	BEGIN
		...
		SET @TOTALMANDATORIES	= COALESCE(@ssseefinal3,0)+COALESCE(@phieebasic2,0)+COALESCE(@pageebasic,0)+COALESCE(@prossseegrossfinal3,0)
...
select [sssemployee] = @ssseefinal3,[sssemployer]=@ssserfinal3,[sssecc]= @sssECCfinal3,[prosssemployee]= @prossseegrossfinal3,[prosssemployer]=@prosssergrossfinal3,[Philhealthee]= @phieebasic2,[pagibigee] = @pageebasic2,[sssprevious] =@forssstotalgrossprevious,[phiprevious] = @forphitotalbasicprevious, [wtaxcompensationfinal] = @WTAXCOMPENSATIONFINAL
