...
CREATE PROCEDURE [dbo].[42bf00bd-6230-462f-a118-a3d93166c3b9V230INDWEEKLYSSSMASS]
...
SELECT employee_id,sssbasis FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment AND employee_id = @idemployee  order by idpayrollsum
   ...
   FETCH NEXT FROM myCursor2 INTO @idemployeesss,@sssbasis
	...
	BEGIN
				...
				--current view
				SELECT @forssstotalgross= COALESCE(Sum(grossamttaxable),0) from payroll_summary WHERE Employee_id = @idemployeesss and  idclientp = @idclient and payrollmonth= @payrollmonth  and  idpayrollsum = @idpayroll -- gross exluded current trx
...
BEGIN TRY
    ...
    SET
        contributionSSSEE  = @ssseefinal3,
        contributionSSSER  = @ssserfinal3,
        contributionSSSECC = @sssECCfinal3,
		basicforsss = 
		case
			when @sssbasis = 'gross' then @forssstotalgrossprevious
			when @sssbasis = 'basic' then @forssstotalbasicprevious
		end
