...
CREATE PROCEDURE [dbo].[42bf00bd-6230-462f-a118-a3d93166c3b9V230INDWEEKLYSSSPRO]
...
SELECT employee_id,sssbasis FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment AND employee_id = @idemployee  order by idpayrollsum
   ...
   FETCH NEXT FROM myCursor8 INTO @proidemployeesss,@prosssbasis
	...
	BEGIN
				...
				--- below is the final computation of sss SSS EMPLOYER basic
				SELECT @prossserbasicfinal = CAST(@prossserbasic as float)- cast(@prossserbasicprevious as float)
...
--select [prosssemployee]= 45689,[prosssemployer]=@prosssergrossfinal3
