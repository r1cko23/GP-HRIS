...
CREATE PROCEDURE [dbo].[42bf00bd-6230-462f-a118-a3d93166c3b9V2manual15]
...
------------------------------------------------------SSS Employer Process 15th Manual-------------------------------------------
...
DECLARE @ssseereference float
DECLARE @forssstotalgross float
DECLARE @forssstotalbasic float

DECLARE @ssser as float
DECLARE @ssserold as float
DECLARE @ssserecom float

DECLARE @ecc as float 
DECLARE @sssECCold FLOAT 
DECLARE @sssECCecom float

DECLARE @sssbasis VARCHAR(20)
 
 
DECLARE myCursor2 CURSOR FOR
-- open payroll summary      

SELECT employee_id,sssbasis FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment  order by Employee_id
   ...
   FETCH NEXT FROM myCursor2 INTO @idemployeesss,@sssbasis
	...
	BEGIN
				...
				SELECT TOP 1 @idpayrollsum2 = idpayrollsum
				...
				SELECT @ssseereference= contributionSSSEE from payroll_summary WHERE employee_id = @idemployeesss and  idclientp = @idclient and Date_Start= @PayrollPeriodStart  
				SELECT TOP 1  @ssser = EmployerSSS FROM SSS where EmployeeSSS <= @ssseereference order by Range desc -- FOR PROPER PROCESSING 
				SELECT TOP 1  @ecc = EmployerECC FROM SSS where EmployeeSSS <= @ssseereference order by Range desc

				--DUSIT 
				SELECT TOP 1  @ssserold = EmployerSSS FROM SSSold where Range <= @forssstotalgross order by Range desc --BASE ON THEIR EXCEL
				SELECT TOP 1  @sssECCold = EmployerECC FROM SSSold where Range <= @forssstotalgross order by Range desc --BASE ON THEIR EXCEL
				
				--ecom
				SELECT TOP 1  @ssserecom = EmployerSSS FROM SSSEmployerecom where Range <= @forssstotalgross order by Range desc --BASE ON THEIR EXCEL
				SELECT TOP 1  @sssECCecom = EmployerECC FROM SSSEmployerecom where Range <= @forssstotalgross order by Range desc --BASE ON THEIR EXCEL
				
					 

				if @idclient = 4 --e-com
				BEGIN
					update payroll_summary set contributionSSSER=@ssserecom,contributionSSSECC =@sssECCecom where idpayrollsum=@idpayrollsum2
				END
				else if @idclient = 10000 
				BEGIN	
					update payroll_summary set contributionSSSER=@ssserold,contributionSSSECC =@ecc where idpayrollsum=@idpayrollsum2
				END
				else --dusit
				begin
					update payroll_summary set contributionSSSER=@ssser,contributionSSSECC =@ecc where idpayrollsum=@idpayrollsum2 --normal
				End
