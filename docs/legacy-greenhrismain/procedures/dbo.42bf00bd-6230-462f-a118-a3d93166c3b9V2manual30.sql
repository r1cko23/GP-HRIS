...
CREATE PROCEDURE [dbo].[42bf00bd-6230-462f-a118-a3d93166c3b9V2manual30]
...
------------------------------------------------------SSS Employer Process 30th manual-------------------------------------------
...
DECLARE @idpayrollsum2sss varchar(20)
...
DECLARE @sssERgrossprevious float 
DECLARE @sssergross as float
DECLARE @sssergross2 float
DECLARE @sssERgrossfinal float

DECLARE @ssseccgrossprevious float
DECLARE @ssseccgross FLOAT 
DECLARE @ssseccgrossfinal FLOAT
...
SELECT employee_id FROM payroll_summary where idclientp = @idclient and Date_Start=@PayrollPeriodStart and department_codep = @idDepartment  order by idpayrollsum
   ...
   FETCH NEXT FROM myCursor2 INTO @idemployeesss
	...
	BEGIN
				...
				SELECT TOP 1  @idpayrollsum2sss =  idpayrollsum FROM payroll_summary where Employee_id = @idemployeesss and idclientp = @idclient and Date_Start= @PayrollPeriodStart  order by grossalary desc
				...
				SELECT  TOP 1 @ssseccgross= EmployerECC from SSS where Range <= @forsssgrandtotalgross order by Range desc
				SELECT @ssseccgrossfinal = CAST(@ssseccgross as float)- cast(@ssseccgross as float)
				...
				--SELECT TOP 1 @sssergross2 = EmployerSSS FROM SSS where EmployeeSSS <= @ssseereference order by Range desc
				...
				SELECT TOP 1  @sssECCecom = EmployerECC FROM SSSEmployerecom where Range <= @forssstotalgross order by Range desc --BASE ON THEIR EXCEL
					...
					if @idclient = 4 
					update payroll_summary set contributionSSSER=@ssserecom,contributionSSSECC =@sssECCecom where idpayrollsum=@idpayrollsum2sss
					else
				-- NORMAL 
					update payroll_summary set contributionSSSER=@sssERgrossfinal,contributionSSSECC =@ssseccgrossfinal where idpayrollsum=@idpayrollsum2sss
