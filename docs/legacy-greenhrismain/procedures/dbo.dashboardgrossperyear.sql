...
-- =============================================
CREATE PROCEDURE [dbo].[dashboardgrossperyear]
...
BEGIN
    ...
    SELECT  
        2023 as yearx,  
        payrollmonth,  
        ROUND(SUM(grossalary), 2) AS totalgross,  
        payrollmonthsort,  
        RIGHT(payrollmonthsort, 2) as finalsort  
    FROM payroll_summary  
    INNER JOIN client ON payroll_summary.idclientp = client.idclient 
	--payrollyear = 2024 and idclient = 105
    WHERE idclientp = @idclient and payrollyear = 2024
...
DECLARE myCursor CURSOR FOR
  ...
  SELECT   2025 as yearx ,payrollmonth ,
  ROUND(SUM(grossalary) , 2)   AS totalgross	
  ,payrollmonthsort ,  CAST(RIGHT(payrollmonthsort, 2) AS INT) AS finalsort
    ...
    client ON payroll_summary.idclientp = client.idclient	
	 WHERE idclientp =  @idclient AND payrollyear = 2025
	--Where payrollyear = 2024 and idclient = 105
	GROUP BY payrollmonth,payrollmonthsort
