...
-- =============================================
CREATE PROCEDURE  [dbo].[payrollselectlistold]
...
FROM            Employee AS e INNER JOIN
				...
				WHERE        (e.department_code = @iddepartment) 
							...
							AND (e.Employee_id IN  (SELECT Employee_id
													...
													AND (Date_End = @previouscuttoff2) AND (department_codep = @iddepartment))) AND (e.fname LIKE N'%' + @keytext + N'%') OR
                         (e.department_code = @iddepartment) AND (e.lname LIKE N'%' + @keytext + N'%') AND (cbp.idclient = @idclient) OR
                         (e.department_code = @iddepartment) AND (cbp.idclient = @idclient) AND (e.fname LIKE N'%' + @keytext + N'%')    
...
SELECT        e.department_code, e.idclient, e.Employee_id, e.fname, e.lname, e.mname, e.SSSno, e.pagibigno, e.philhealthno, e.TINno, e.status, e.tax_status, cbp.jobposition, e.Position1, d.Department_desc, e.departmentsub, 
                         e.paythrough, e.bankaccountno, c.companyname, e.bankname, e.tagdelete
FROM            Employee AS e INNER JOIN
                         client_branch_position AS cbp ON e.Position1 = cbp.idbranchposition INNER JOIN
                         Department AS d ON e.department_code = d.iddepartment INNER JOIN
...
WHERE        (e.department_code = @iddepartment)
			AND (cbp.idclient = @idclient) 
			AND (e.status = 'Active') 
			AND (e.Employee_id NOT IN(SELECT TOP (1) Employee_id
                               FROM  payroll_summary AS ps
                               WHERE        (Employee_id = e.Employee_id) AND (idclientp = e.idclient) AND (Date_Start = @datestart))) AND (e.tagdelete = N'N') AND (e.fname LIKE N'%' + @keytext + N'%') AND (e.lname LIKE N'%' + @keytext + N'%') OR
                         (e.department_code = @iddepartment) AND (cbp.idclient = @idclient) AND (e.fname LIKE N'%' + @keytext + N'%') OR
                         (e.department_code = @iddepartment) AND (cbp.idclient = @idclient) AND (e.lname LIKE N'%' + @keytext + N'%')
			ORDER BY e.lname, e.fname
