CREATE PROCEDURE [dbo].[sp_payslipwithvalue]
    @idclient INT, 
    @iddepartment INT, 
    @uname NVARCHAR(20), 
    @datestart DATE,
...
BEGIN
    ...
    CROSS APPLY (VALUES
		...
		('Legal Holiday2',ps.LegalHoliday2_Hours,      ps.legalholiday2),
		...
		('Special Holiday2',ps.specialholiday2_hours,  ps.specialholiday2),
    ...
    WHERE ps.idclientp = @idclient 
      AND ps.department_codep = @iddepartment
      AND ps.Date_Start = @datestart
	  AND ps.Date_End = @dateend
      AND v.amount <> 0
      AND NOT EXISTS (
            SELECT 1 
            FROM otchart o
