...
-- ===========================================
CREATE PROCEDURE [dbo].[usp_Accrual_InsertExcelData]
	@Guid nvarchar(60)
	,@idClient int
...
BEGIN
	...
	IF (ISNULL(@DupsTag, 0) = 0)
	BEGIN
		-- EmpId is existing, but with DIFFERENT Name: 
		SELECT @DupsTag = -1, @ToolTipText = 'Employee ID: ' + CAST(@EmployeeId AS VarChar(10)) + ' with different Name!' 
		WHERE (SELECT TOP 1 COUNT(Employee_Id) FROM Employee WHERE tagdelete = 'N' AND Employee_id = @EmployeeId AND LEFT(LNAME,3) LIKE LEFT(@Name,3))=0
	END	
