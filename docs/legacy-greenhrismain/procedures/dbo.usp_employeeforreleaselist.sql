...
-- =============================================
CREATE PROCEDURE [dbo].[usp_employeeforreleaselist]
@filtertext5 varchar(20)
...
WHERE e.finalpaystatus IN ('Release', 'For Release');
...
SELECT employeeid,fname,lname,mname,lastpayroll, noofdays,employementtype,status,gender,
    companyname,
    departmentname,
	datehired
FROM #TempTable
where (
        ISNULL(@filtertext5, '') = ''
        OR fname LIKE '%' + @filtertext5 + '%'
        OR lname LIKE '%' + @filtertext5 + '%'
        OR mname LIKE '%' + @filtertext5 + '%'
        OR companyname LIKE '%' + @filtertext5 + '%'
        OR departmentname LIKE '%' + @filtertext5 + '%'
    );
