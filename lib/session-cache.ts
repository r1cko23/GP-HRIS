/**
 * Browser session cache (memory + sessionStorage) for instant page remount paint.
 * Hydration-safe: callers must restore via useEffect, not useState initializers.
 */

const STORAGE_PREFIX = "gp:sc:";
const VERSION_KEY = "gp:sc:__v";
...
export async function sessionLoadJson<T>(
  key: string,
  loader: () => Promise<T>
): Promise<T> {
...
export function prefetchSessionLoad<T>(
  key: string,
  loader: () => Promise<T>
): void {
  if (typeof window === "undefined") return;
  if (sessionCacheGet(key)) return;
  void sessionLoadJson(key, loader).catch(() => undefined);
}
