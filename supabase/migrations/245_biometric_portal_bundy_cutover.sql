-- Portal bundy cutover (deferred in 244): mapped staff use MB10 only.
-- Phone GPS bundy must not write IN/OUT. Portal gates via employee_has_biometric_map.

CREATE OR REPLACE FUNCTION public.employee_has_biometric_map(p_employee_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    SELECT 1
    FROM public.biometric_user_maps
    WHERE employee_id = p_employee_id
  );
$$;

COMMENT ON FUNCTION public.employee_has_biometric_map(uuid) IS
  'True when employee has a biometric PIN map — portal hides GPS bundy.';

GRANT EXECUTE ON FUNCTION public.employee_has_biometric_map(uuid) TO anon, authenticated;
...
BEGIN
  ...
  IF public.employee_has_biometric_map(p_employee_id) THEN
